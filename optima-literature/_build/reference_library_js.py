...
(function(){
  ...
  /* ---------------- teacher bookshelves ----------------
     ...
     A teacher's own shelves live in localStorage and render as the same tiles:
     under My Classroom (with Edit and Remove) and under Teacher Bookshelves
     (badged "My shelf"). The tile IS what a teacher came for. The classrooms.json
     record that carries a shelf to every other teacher is a maintainer concern,
     so it renders only while the Data checks gear is on. */
  ...
  function tbShelfCard(s, mine, idx, editable){
    var ids = tbEntries(s);
    ...
    return '<div class="tbshelf' + (mine ? ' tbmine' : '') + '">'
      + '<div class="tbsh"><div class="tbsteach">' + esc(s.teacher)
      + (mine && !editable ? ' <span class="bdg tg">My shelf</span>' : '')
      ...
      + (editable ? '<span class="tbsp"></span>'
                  + '<button class="tick" data-tbedit="' + idx + '">Edit</button>'
                  + '<button class="tick" data-tbdel="' + idx + '">Remove</button>' : '')
      + '</div></div>';
  }
  ...
  // Walks up from a node to the nearest ancestor carrying a class. Written out
  // on purpose: .closest() is banned in this file (see the header comment).
  function tbUp(node, cls){
    var n = node ? node.parentElement : null;
  ...
  function tbPrintShelf(el){
    if (!el) return;
    // A tile under My Classroom sits inside a step card that the print
    // stylesheet hides; mark that wrapper so the tile still reaches paper.
    var wrap = tbUp(el, 'tbcard');
    function cleanup(){
      document.body.className = document.body.className.replace(' tbprinting', '');
      el.className = el.className.replace(' tbprintme', '');
      if (wrap) wrap.className = wrap.className.replace(' tbprintwrap', '');
      if (window.onafterprint === cleanup) window.onafterprint = null;
    }
    el.className = el.className + ' tbprintme';
    if (wrap) wrap.className = wrap.className + ' tbprintwrap';
  ...
  // Print and copy on a shelf tile, wherever the tile is drawn. Print takes
  // the tile element; copy takes the shelf record from the rows just rendered.
  function tbWireShelfButtons(box, rows){
    var pb = box.querySelectorAll('[data-tbprint]');
    for (var p=0;p<pb.length;p++){
      pb[p].addEventListener('click', (function(btn){
        return function(){ tbPrintShelf(tbUp(btn, 'tbshelf')); };
      })(pb[p]));
    }
    ...
    for (var c=0;c<cb.length;c++){
      cb[c].addEventListener('click', (function(btn){
        return function(){
          var row = rows[parseInt(btn.getAttribute('data-tbcopy'), 10)];
  ...
  function viewMine(){
    ...
    var picker = '<div class="tbcard"><div class="tbhead"><span class="tbnum">2</span>'
      ...
      + '<button class="tbbtn pri" id="tbSave" disabled>Save to my bookshelf</button>'
      + '</span></div></div>';

    var mineCard = '<div class="tbcard tbhide" id="tbMineCard">'
      + '<div class="tbhead"><span class="tbnum">3</span>'
      + '<span class="tbname">My bookshelf</span>'
      + '<span class="tbnote" id="tbMineNote"></span></div>'
      + '<div class="tbshelves tbminegrid" id="tbMineGrid"></div>'
      + '<div class="tbrun"><span class="tbrunL">Each tile is one course. Print it, or '
      + 'copy the list to paste into a syllabus or a note to families.</span>'
      + '<span class="tbright"><button class="tbbtn gh" id="tbAnother">Add another '
      + 'course</button></span></div></div>';

    // Maintainer-only. Hidden unless the Data checks gear is on (body.show-admin).
    var outCard = '<div class="tbcard tbadmin tbhide" id="tbOutCard">'
      + '<div class="tbhead"><span class="tbnum">&#9881;</span>'
      + '<span class="tbname">For the library maintainer</span>'
      + '<span class="tbnote"><button class="tick" id="tbCopyJson">Copy this block'
      + '</button></span></div>'
      + '<div class="tbbody"><div class="tbfile"><pre id="tbJson"></pre></div>'
      + '<div class="tbsay"><b>What this is.</b> The shelves above as records for '
      + '<b>classrooms.json</b>, one per course. Paste them into that file and rebuild '
      + 'the library; the shelves then appear for every teacher under Teacher '
      + 'Bookshelves. This card shows only while Data checks is on. Teachers do not '
      + 'see it and do not need it.</div>'
      + '</div></div>';

    return shell('My Classroom',
      'Build the bookshelf for your course in three steps: name the course, tick the '
      + 'titles you teach, and save. Your shelf appears below as a tile you can print or '
      + 'copy, with each title&rsquo;s edition and its purchase or free-text links already '
      + 'attached. Teach more than one course? Save the first, then add another; your '
      + 'name carries over. Your shelves are kept in this browser and are here the next '
  ...
  function tbSync(){
    ...
    if (save){
      ...
      save.textContent = (tbEdit >= 0) ? 'Update this course' : 'Save to my bookshelf';
    }
  ...
  function tbDrawMine(){
    ...
    var list = document.getElementById('tbMineGrid');
    if (!card || !list) return;
    if (!me.courses.length){
      list.innerHTML = '';
      card.className = 'tbcard tbhide'; tbDrawJson(); return;
    }
    card.className = 'tbcard';
    var rows = [], h = '';
    for (var i=0;i<me.courses.length;i++){
      var s = me.courses[i];
      rows.push({s: s, draft: false});
      h += tbShelfCard(s, true, i, true);
    }
    list.innerHTML = h;
    if (tbEdit >= 0){
      var tiles = list.querySelectorAll('.tbshelf');
      if (tiles[tbEdit]) tiles[tbEdit].className += ' tbediting';
    }
    tbWireShelfButtons(list, rows);
  ...
  function tbDrawJson(){
    ...
    if (!card || !pre) return;
    if (!me.courses.length){ card.className = 'tbcard tbadmin tbhide'; return; }
  ...
  function tbDrawShelves(){
    ...
    if (!rows.length){
      h = '<div class="tbblank">' + (all.length
        ...
        : 'No bookshelves yet. Build yours under <b>My Classroom</b> and it appears '
          + 'here.') + '</div>';
    }
    box.innerHTML = h;
  ...
  adminToggle.addEventListener('click', function(){
    ...
    if (open){
      ...
      document.body.className = document.body.className.replace(' show-admin', '');
      ...
      document.body.className = document.body.className + ' show-admin';
